...
const platesBetweenCandles = function(s, queries) {
  const ans = []; const preSum = new Array(s.length).fill(0)
  ...
  const rightArr = new Array(s.length).fill(0)
  // 等同于向上取整
  ...
  for (let i = 0; i < queries.length; i++) {
    ...
    const left = rightArr[queries[i][0]]; const right = leftArr[queries[i][1]]
