...
export function validUtf8(data: number[]): boolean {
  ...
  for (let i = 0; i < transform.length; i++) {
    const temp = transform[i]
    if (temp.startsWith('0')) {
      if (!testSingleBinary(temp))
        return false
    }
    else {
      const cnt: number = temp.search('0')
      if (cnt >= 5 || cnt <= 1)
        return false
      const arr: string[] = transform.slice(i, i + cnt)
      i += (cnt - 1)
      if (i >= transform.length)
        return false
      if (!testMultipleBinary(arr, cnt))
        return false
    }
  }
  return true
...
function testMultipleBinary(numStrArr: string[], n: number): boolean {
  const str: string = '1'.repeat(n)
  if (!numStrArr[0].startsWith(str))
    return false
  for (let i = 1; i < n; i++) {
    if (!numStrArr[i].startsWith('10'))
      return false
  }
...
function toBinary(num): string {
  let binary = String(parseInt(num).toString(2))
